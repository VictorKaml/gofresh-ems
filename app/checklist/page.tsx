...
export const DailyChecklist: React.FC<Props> = ({
  ...
  }, [employees, selectedEmployeeIds, showOnlySelected, selectedDept, selectedCostCenter, searchQuery]);
  ...
  const employeesWithCompleteRecords = useMemo(() => {
    ...
    selectedEmployeeList.forEach(emp => {
      ...
      if (hasIn && hasOut) {
        ...
        const recordWithOperator = empRecords.find(r => r.adjusted_by || r.adjustedBy || r.operatorEmail || r.operator_email || r.created_by || r.updated_by);
    ...
    return conflicts;
  }, [selectedDate, selectedEmployeeList, attendanceRecords, selectedEmployeeIds]);
  ...
  const isAllFilteredSelected = useMemo(() => {
    ...
    return searchMatchedEmployees.every(emp => selectedEmployeeIds.has(String(emp.staffCode || emp.id)));
  ...
  const toggleSelectAllFiltered = () => {
    const matchedIds = searchMatchedEmployees.map(emp => String(emp.staffCode || emp.id));
  ...
  const handleBatchConfirmAttendance = async () => {
    if (selectedEmployeeList.length === 0 || !selectedDate || hasConflictingRecords) return;

    if (isSelectedDateInFuture) {
      setNotification({ type: 'error', message: 'Future dates cannot be logged.' });
      return;
    }

    if (!auditReason.trim()) {
      setNotification({ type: 'error', message: 'Please select a reason before confirming attendance.' });
      return;
    }
    ...
    const formattedInTime = checkInTime.length === 5 ? `${checkInTime}:00` : checkInTime;
    const formattedOutTime = checkOutTime.length === 5 ? `${checkOutTime}:00` : checkOutTime;
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(resData.error || 'Failed to post manual attendance records.');
      }
      ...
      setNotification({ 
        type: 'error', 
        message: err.message || 'An error occurred while submitting attendance.' 
      });
    } finally {
  ...
  return (
    <div className="flex flex-col gap-6 p-6 bg-gray-50 min-h-screen relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-4 rounded-lg shadow-sm border border-gray-200 gap-4">
        <div>
          <h2 className="text-xl font-bold text-gray-800">Daily Attendance Checklist</h2>
          <p className="text-sm text-gray-500">Batch manage daily workspace attendance for multiple employees</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button 
            onClick={() => onDownloadExceptionReport && onDownloadExceptionReport('Late Arrivals')}
          ...
          <button 
            onClick={() => onDownloadOverallAnalytics && onDownloadOverallAnalytics(selectedDept, selectedCostCenter)}
      ...
      {/* Notification Banner */}
      {notification && (
        <div className={`p-4 rounded-md flex items-center gap-2 ${notification.type === 'success' ? 'bg-green-50 text-green-800 border border-green-200' : 'bg-red-50 text-red-800 border border-red-200'}`}>
          ...
          <div className="p-4 border-b border-gray-100 space-y-3">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {isAllFilteredSelected ? "Deselect Filtered" : `Select All Filtered (${searchMatchedEmployees.length})`}
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between gap-2 pt-1">
              ...
              >
                ...
                {showOnlySelected ? "Show All Employees" : `Show Selected Only (${selectedEmployeeIds.size})`}
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-gray-100">
            {displayedEmployees.length === 0 ? (
              ...
              displayedEmployees.map((employee) => {
                ...
                const conflictObj = employeesWithCompleteRecords.find(c => String(c.employee.staffCode || c.employee.id) === empId);
                  ...
                  <div
                    ...
                    className={`p-4 cursor-pointer transition-colors flex items-center justify-between hover:bg-gray-50 ${
                      conflictObj 
                        ...
                        : isChecked ? 'bg-blue-50/80 border-l-4 border-blue-600' : ''
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">ID: {empId} • {employee.department || 'N/A'}</p>
                        {conflictObj ? (
                          <p className="text-[11px] text-red-700 font-medium mt-0.5">
                            Confirmed By: <strong>{conflictObj.confirmedBy}</strong>
                          </p>
                        ) : (
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="flex justify-between items-center pb-4 border-b border-gray-100">
              ...
              <div className="text-right">
                <span className="text-xs text-gray-400 block">Target Log Date</span>
              ...
              <div className="p-4 bg-red-50 border border-red-200 rounded-md text-red-800 space-y-2">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <h4 className="font-bold text-sm text-red-900">
                      Cannot Proceed: Complete Record Already Exists ({employeesWithCompleteRecords.length})
                    </h4>
                    <p className="text-xs text-red-700 mt-0.5">
                      The following employee(s) already have a complete IN/OUT attendance record on <strong>{selectedDate}</strong> and cannot be updated again in this batch. Please remove them from the selection list to proceed.
                    </p>
                  </div>
                </div>
                ...
                <div className="max-h-28 overflow-y-auto divide-y divide-red-200 border border-red-200 rounded bg-white text-xs">
                  {employeesWithCompleteRecords.map((c) => {
                    ...
                    return (
                      <div key={cId} className="p-2 flex justify-between items-center text-red-900">
                        <div>
                          <span className="font-semibold">{c.employee.fullName || c.employee.name}</span>
                          <span className="text-gray-500 ml-1">({cId})</span>
                        </div>
                        <div className="text-right">
                          <span className="text-[11px] text-gray-600 block">Confirmed By:</span>
                          <span className="font-medium text-red-800">{c.confirmedBy}</span>
                        </div>
                      </div>
                    );
                  })}
            ...
            <div>
              ...
              <div className="grid grid-cols-7 gap-2 text-center">
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
                  <span key={day} className="text-xs font-semibold text-gray-400 py-1">{day}</span>
                ))}
                {daysInMonth.map((day) => {
                  ...
                  if (isFutureDate) {
                    dayStyle = 'opacity-40 cursor-not-allowed bg-gray-50 text-gray-400';
                  } else if (isSelected) {
                    dayStyle = 'bg-blue-600 text-white shadow-sm ring-2 ring-blue-300';
                  } else if (isToday) {
                    dayStyle = 'bg-blue-50 text-blue-600 border border-blue-200 font-semibold';
                  }
            ...
            {isSelectedDateInFuture && (
              <div className="p-3 bg-amber-50 border border-amber-200 text-amber-800 rounded-md text-xs font-medium flex items-center gap-2">
                ...
                <span>Attendance cannot be logged for future dates ({selectedDate}). Please select today or a past date.</span>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input 
                  type="time" 
                  disabled={isSelectedDateInFuture || selectedEmployeeIds.size === 0 || hasConflictingRecords}
                  value={checkInTime}
            ...
            <div className="pt-2">
              <button
                ...
                disabled={selectedEmployeeIds.size === 0 || isSelectedDateInFuture || hasConflictingRecords}
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-xl border border-gray-200 max-w-lg w-full p-6 flex flex-col gap-4 animate-in fade-in zoom-in-95">
            ...
            <div className="bg-gray-50 p-3 rounded-md border border-gray-200 text-xs text-gray-700 space-y-1">
              ...
              <p><strong>Override Reason:</strong> {auditReason}</p>
              <p><strong>Adjusted By Operator:</strong> {operatorEmail || "System Admin"}</p>
            </div>
            ...
            <div>
              ...
              <div className="max-h-48 overflow-y-auto divide-y border rounded-md divide-gray-100 bg-white">
                {selectedEmployeeList.map((emp) => (
                  <div key={emp.staffCode || emp.id} className="p-2.5 flex justify-between items-center text-xs">
                    <div>
                      <span className="font-semibold text-gray-800">{emp.fullName || emp.name}</span>
                      <span className="text-gray-400 ml-2">({emp.staffCode || emp.id})</span>
                    </div>
                    <span className="text-gray-500">{emp.department || 'N/A'}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-2 border-t">
              ...
              >
                {isSubmitting ? "Posting Records..." : "Confirm & Submit Attendance"}
              </button>
            </div>
          </div>
        </div>
