...
// Helper function to convert any standard time string (HH:MM or HH:MM:SS) or Date object to absolute minutes
  ...
  try {
    ...
    if (!staffCode) {
      return NextResponse.json(
        { error: "Missing staffCode parameter" },
        { status: 400 },
      );
    }
    ...
    // 2️⃣ Chronological Sort in-memory
    const logs = [...rawLogs].sort((a, b) => {
      // Compare native JavaScript Date objects using timestamps
      const dateCompare = a.date.getTime() - b.date.getTime();
      if (dateCompare !== 0) return dateCompare;

      // Compare time objects using timestamps as well
    ...
    const processedRecords = uniqueDates.map((dateStr) => {
      ...
      if (rawCheckIn !== "—" && rawCheckOut !== "—") {
        ...
        if (diffMinutes > 0) {
          ...
          const standardLimitMinutes =
            weekDay === "Saturday" || weekDay === "Sunday" ? 330 : 510;

          if (diffMinutes > standardLimitMinutes) {
            overtimeHours = parseFloat(
              ((diffMinutes - standardLimitMinutes) / 60).toFixed(2),
            );
          }
        }
      } else {
      ...
      const formatDisplayTime = (rawTime: string | Date) => {
        if (rawTime === "—") return "—";
