...
export default function EMSDashboard() {
  ...
  const [newStaffWorkType, setNewStaffWorkType] = useState<"In Office" | "Remote" | "Hybrid">("In Office");
  ...
  const handleDeleteSystemUser = async (id: string) => {
    if (!confirm("Are you sure you want to permanently revoke system access for this user?")) return;
    try {
      const res = await fetch(`/api/system-users?id=${encodeURIComponent(id)}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (res.ok) {
    ...
    async function initializeDashboard() {
      try {
        ...
        setEmployeeDirectory([
          { staffCode: "GF001", fullName: "CHIMWEMWE PHIRI", email: "c.phiri@gofresh.com", designation: "Supervisor", department: "Operations", costCenter: "Main Barn", status: "Active", workType: "In Office" },
          { staffCode: "GF002", fullName: "ATUPELE BANDAL", email: "a.bandal@gofresh.com", designation: "General Manager", department: "Administration", costCenter: "Front Office", status: "Active", workType: "Hybrid" },
          { staffCode: "GF003", fullName: "BRIAN CHIKWA", email: "b.chikwa@gofresh.com", designation: "Maintenance Lead", department: "Engineering", costCenter: "Workshop", status: "Active", workType: "In Office" },
          { staffCode: "GF004", fullName: "SARAH CHEN", email: "sarah@company.com", designation: "UX Designer", department: "Design", costCenter: "HQ", status: "Active", workType: "Hybrid" },
          { staffCode: "GF005", fullName: "DANIEL KIM", email: "d.kim@company.com", designation: "DevOps Engineer", department: "Engineering", costCenter: "Remote Hub", status: "Inactive", workType: "Remote" }
        ]);
      } finally {
        setIsLoading(false);
      }
    ...
    async function syncAttendanceData() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setRawSwipesBuffer([
            { id: "GF001", date: todayString, weekDay: "Today", time: "07:15", type: "Check In" },
            { id: "GF001", date: todayString, weekDay: "Today", time: "16:30", type: "Check Out" },
            { id: "GF002", date: todayString, weekDay: "Today", time: "07:48", type: "Check In" },
            { id: "GF002", date: todayString, weekDay: "Today", time: "17:00", type: "Check Out" },
            { id: "GF003", date: todayString, weekDay: "Today", time: "08:10", type: "Check In" },
            { id: "GF004", date: todayString, weekDay: "Today", time: "07:25", type: "Check In" }
          ]);
  ...
  const handleCreateStaffSubmit = (e: React.FormEvent) => {
    ...
    setBavStatus({ type: "success", message: `Successfully added ${payloadProfile.fullName} to the staff list!` });
  ...
  const toggleDailyPresence = async (staffCode: string, isCurrentlyPresent: boolean) => {
    ...
    if (isCurrentlyPresent) {
      ...
      setRawSwipesBuffer((prev) => prev.filter((s) => !(s.id === staffCode && s.date === dateStr)));
      ...
      try {
        ...
        addLog(`Marked ${staffCode} absent for ${dateStr} and synced the removal to Supabase.`);
      } catch (err) {
        console.error(err);
        setRawSwipesBuffer(previousSwipes);
        addLog(`Could not sync absence for ${staffCode}. Reverted the checklist locally.`);
      } finally {
    ...
    const punchPair: RawSwipe[] = [
      { id: staffCode, date: dateStr, weekDay, time: "07:30", type: "Check In" },
      { id: staffCode, date: dateStr, weekDay, time: "16:30", type: "Check Out" },
    ];

    const previousSwipes = rawSwipesBuffer;
    setRawSwipesBuffer((prev) => [...prev.filter((s) => !(s.id === staffCode && s.date === dateStr)), ...punchPair]);

    try {
      const response = await fetch("/api/attendance", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ staffCode, date: dateStr, weekDay, punches: punchPair }),
      });
      if (!response.ok) throw new Error("Save request failed");
      addLog(`Marked ${staffCode} present for ${dateStr} (07:30 - 16:30) and synced to Supabase.`);
    } catch (err) {
      console.error(err);
      setRawSwipesBuffer(previousSwipes);
      addLog(`Could not sync attendance for ${staffCode}. Reverted the checklist locally.`);
    } finally {
      setSavingChecklistCode(null);
    }
  };
  ...
  const systemProcessedDataset = useMemo(() => {
    ...
    const uniqueDates = foundDates.length > 0 ? foundDates.sort() : [new Date().toISOString().split("T")[0]];
  ...
  const liveMetricsRollup = useMemo(() => {
    ...
    systemProcessedDataset.forEach((emp) => {
      ...
      if (todaySwipes.length > 0) {
        const checkIns = todaySwipes.filter((s) => s.type.toLowerCase().includes("in")).sort((a, b) => a.time.localeCompare(b.time));
        const checkOuts = todaySwipes.filter((s) => s.type.toLowerCase().includes("out")).sort((a, b) => a.time.localeCompare(b.time));
  ...
  const staffModuleDataset = useMemo(() => {
    return systemProcessedDataset.filter((row) => {
      const textMatch = row.fullName.toLowerCase().includes(staffSearchQuery.toLowerCase()) ||
      ...
      if (staffSubTab === "INACTIVE") return textMatch && row.status === "Inactive";
      return textMatch;
    });
  ...
  const checklistPresentCount = useMemo(
    () => dailyChecklistDataset.filter((e) => e.isPresent).length,
  ...
  const allHistoricalSwipes = useMemo(() => {
    return [...rawSwipesBuffer].sort((a, b) => b.date.localeCompare(a.date) || b.time.localeCompare(a.time));
  }, [rawSwipesBuffer]);
    ...
    reader.onload = async (evt) => {
      try {
        ...
        if (isBiometricLog) {
          ...
          addLog(`Sending ${dynamicSwipes.length} clean biometric entries to database cluster...`);
          ...
          if (response.ok && result.success) {
            ...
            addLog("Fresh biometric timecards added to calculation pools and persisted successfully.");
          } else {
            setBavStatus({ 
              type: "error", 
              message: `Sync Failed: ${result.error || "Database transmission rejected."}` 
            });
            addLog(`[DATABASE ERROR]: ${result.details || "Check server log tags"}`);
          }
        } else {
          setBavStatus({ type: "error", message: "Invalid format. 'Card Swiping Type' headers missing." });
        }
      } catch (err) {
        setBavStatus({ type: "error", message: "Could not read this Excel file. Make sure it contains proper timecard sheets." });
      } finally {
  ...
  const generateEmployeeProfilePDF = (emp: any) => {
    try {
      ...
      doc.setFontSize(14);
      doc.setTextColor(15, 23, 42);
      doc.text("Accumulated Cumulative Operational Metrics", 14, (doc as any).lastAutoTable.finalY + 15);

      const analyticsMetadata = [
        ["Total Regular Hours Logged", `${emp.metrics.totalRegularHours} hrs`],
        ["Total Logged Overtime Hours", `+${emp.metrics.totalOvertimeHours} hrs`],
        ["Gross Accumulated Combined Shifts", `${emp.metrics.totalHoursSum} hrs`],
        ["Validated Active Attendance Days", `${emp.metrics.presentDaysCount} days`]
      ];
      ...
      doc.save(`Profile_Report_${emp.staffCode}_${emp.fullName.replace(/\s+/g, "_")}.pdf`);
      addLog(`Safely downloaded local client data PDF ledger card for ${emp.fullName}`);
    } catch (e) {
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-xs">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <input type="file" ref={fileInputRef} onChange={handleFileUploadDispatch} accept=".xlsx, .xls, .csv" className="hidden" id="header-file-uploader" />
            <Button asChild variant="outline" className="h-9 border-blue-200 text-blue-600 text-xs font-bold cursor-pointer">
        ...
        <main className="flex-1 max-w-7xl w-full mx-auto p-6 space-y-6">
          ...
          {activeTab === "OVERVIEW" && (
            <>
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-2">
                <div>
                  <h2 className="text-base font-extrabold text-slate-800 uppercase tracking-wide">Workforce Attendance Analytics Dashboard</h2>
                  <p className="text-xs text-slate-400 font-medium uppercase">Select target transaction dates below to parse automatic card metric accumulations.</p>
                </div>
                <div className="flex items-center gap-3 bg-white border border-slate-200 p-2 rounded-xl shadow-xs shrink-0">
                  <label htmlFor="dashboard-date-filter" className="text-[10px] font-black uppercase text-slate-400 pl-1 tracking-wider">Query Calendar:</label>
                  <Input
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <Card className="bg-white border border-slate-200 rounded-xl border-l-4 border-l-blue-600 shadow-xs">
                  <CardHeader className="p-4">
                    ...
                    <CardTitle className="text-xl font-black text-slate-900 mt-1">{liveMetricsRollup.onsite} Checked In</CardTitle>
                  </CardHeader>
                </Card>
                ...
                <Card className="bg-white border border-slate-200 rounded-xl border-l-4 border-l-amber-500 shadow-xs">
                  <CardHeader className="p-4">
                    <CardDescription className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                      <AlertTriangle className="w-4 h-4 text-amber-500" /> Forgot to Clock Out or In
                    </CardDescription>
                    <CardTitle className="text-xl font-black text-slate-900 mt-1">{liveMetricsRollup.missingClocks} Workers</CardTitle>
                  </CardHeader>
                </Card>
                ...
                <Card className="bg-white border border-slate-200 rounded-xl border-l-4 border-l-emerald-500 shadow-xs">
                  <CardHeader className="p-4">
                    <CardDescription className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                      <CheckCircle2 className="w-4 h-4 text-emerald-500" /> On Time Today (Before 07:30 AM)
                    </CardDescription>
                    <CardTitle className="text-xl font-black text-emerald-600 mt-1">{liveMetricsRollup.onTime} Workers</CardTitle>
                  </CardHeader>
                </Card>
                ...
                <Card className="bg-white border border-slate-200 rounded-xl border-l-4 border-l-rose-500 shadow-xs">
                  <CardHeader className="p-4">
                    <CardDescription className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                      <Clock className="w-4 h-4 text-rose-500" /> Late Today (After 07:30 AM)
                    </CardDescription>
                    <CardTitle className="text-xl font-black text-rose-600 mt-1">{liveMetricsRollup.late} Workers</CardTitle>
                  </CardHeader>
                </Card>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <Card className="lg:col-span-2 bg-white border border-slate-200 rounded-xl shadow-xs">
                  <CardHeader className="p-4">
                    <CardTitle className="text-xs font-black text-slate-400 uppercase tracking-widest">Total Hours Worked Graph</CardTitle>
                  </CardHeader>
                  <CardContent className="h-60 p-2">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={systemProcessedDataset}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                        <XAxis dataKey="fullName" stroke="#94a3b8" fontSize={9} />
                        <YAxis stroke="#94a3b8" fontSize={9} />
                        <Tooltip />
                        <Bar dataKey="metrics.totalHoursSum" name="Hours Accumulation" fill="#2563eb" radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </CardContent>
                </Card>
                ...
                <Card className="bg-white border border-slate-200 rounded-xl shadow-xs">
                  <CardHeader className="p-4">
                    <CardTitle className="text-xs font-black text-rose-500 uppercase tracking-widest">Workers With No Hours Logged</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-2 max-h-60 overflow-y-auto p-4 pt-0">
                    {liveMetricsRollup.notCapturingList.length === 0 ? (
                      <span className="text-xs text-slate-400 font-medium uppercase block py-2">All employees have logged card entries safely.</span>
                    ) : (
                      liveMetricsRollup.notCapturingList.map((name, idx) => (
                        <div key={idx} className="p-2.5 bg-rose-50 rounded-lg border border-rose-100 text-xs font-bold text-rose-800 flex justify-between items-center">
                          <span>{name}</span>
                          <Badge variant="destructive" className="text-[9px]">0 HOURS</Badge>
                        </div>
                      ))
                    )}
                  </CardContent>
          ...
          {activeTab === "CHECKLIST" && (
            <>
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-2">
                <div>
                  <h2 className="text-base font-extrabold text-slate-800 uppercase tracking-wide">Daily Attendance Checklist</h2>
                  <p className="text-xs text-slate-400 font-medium uppercase">Tick a box to log presence. A standard 07:30 - 16:30 punch pair is generated automatically.</p>
                </div>
                <div className="flex items-center gap-3 bg-white border border-slate-200 p-2 rounded-xl shadow-xs shrink-0">
                  <label htmlFor="checklist-date-filter" className="text-[10px] font-black uppercase text-slate-400 pl-1 tracking-wider">Checklist Date:</label>
                  <Input
              ...
              <Card className="bg-white border border-slate-200 rounded-xl shadow-xs">
                <CardHeader className="p-4 bg-slate-50/60 border-b border-slate-100 flex flex-row items-center justify-between flex-wrap gap-3">
                  <div>
                    <CardTitle className="text-xs font-black text-slate-500 uppercase tracking-widest">
                      Roster for {selectedDate}
                    </CardTitle>
                    <div className="text-[10px] text-slate-400 font-bold uppercase mt-1">
                      {checklistPresentCount} of {dailyChecklistDataset.length} staff marked present
                    </div>
                  </div>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-slate-50 border-b border-slate-200">
                        <TableHead className="text-[10px] font-black uppercase text-slate-500 w-12">Present</TableHead>
                        <TableHead className="text-[10px] font-black uppercase text-slate-500">ID Code</TableHead>
                        <TableHead className="text-[10px] font-black uppercase text-slate-500">Full Name</TableHead>
                        <TableHead className="text-[10px] font-black uppercase text-slate-500">Department</TableHead>
                        <TableHead className="text-[10px] font-black uppercase text-slate-500 text-right">Check In</TableHead>
                        <TableHead className="text-[10px] font-black uppercase text-slate-500 text-right">Check Out</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {dailyChecklistDataset.length === 0 ? (
                        <TableRow>
                          <TableCell colSpan={6} className="text-center text-xs text-slate-400 font-bold uppercase py-8">
                            No staff match this search.
                          </TableCell>
                        </TableRow>
                      ) : (
                        dailyChecklistDataset.map((emp) => {
                          const isSaving = savingChecklistCode === emp.staffCode;
                          return (
                            <TableRow key={emp.staffCode} className="border-b border-slate-100 hover:bg-slate-50/50">
                              <TableCell>
                                <button
                                  type="button"
                                  disabled={isSaving}
                                  onClick={() => toggleDailyPresence(emp.staffCode, emp.isPresent)}
                                  aria-pressed={emp.isPresent}
                                  aria-label={`Mark ${emp.fullName} ${emp.isPresent ? "absent" : "present"} for ${selectedDate}`}
                              ...
                              <TableCell className="text-xs text-right font-medium">{emp.clockIn}</TableCell>
                              <TableCell className="text-xs text-right font-medium">{emp.clockOut}</TableCell>
                            </TableRow>
                          );
                        })
                      )}
          ...
          {activeTab === "STAFF_PANEL" && (
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-2xs">
                <div>
                  <h1 className="text-xl font-black text-slate-900 tracking-tight">Staff Workspace</h1>
                  <p className="text-xs text-slate-400 font-semibold uppercase">{employeeDirectory.length} total employees registered in system ecosystem</p>
                </div>
                <Button onClick={() => setIsAddModalOpen(true)} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs uppercase px-4 h-10 tracking-wider flex items-center gap-2 rounded-lg">
                  <UserPlus className="w-4 h-4" /> Add Staff Member
                </Button>
              </div>

              <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 pb-2">
                <button onClick={() => setStaffSubTab("REGISTER")} className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase transition-all flex items-center gap-1.5 ${staffSubTab === "REGISTER" ? "bg-blue-600 text-white shadow-xs" : "bg-white text-slate-600 border border-slate-200 hover:bg-slate-50"}`}>
                  <Briefcase className="w-3.5 h-3.5" /> Employee Register
                </button>
                <button onClick={() => setStaffSubTab("ACTIVE")} className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase transition-all flex items-center gap-1.5 ${staffSubTab === "ACTIVE" ? "bg-emerald-600 text-white shadow-xs" : "bg-white text-slate-600 border border-slate-200 hover:bg-slate-50"}`}>
                  <UserCheck className="w-3.5 h-3.5" /> Active Employees
                </button>
                <button onClick={() => setStaffSubTab("INACTIVE")} className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase transition-all flex items-center gap-1.5 ${staffSubTab === "INACTIVE" ? "bg-rose-600 text-white shadow-xs" : "bg-white text-slate-600 border border-slate-200 hover:bg-slate-50"}`}>
                  <UserX className="w-3.5 h-3.5" /> Inactive Employees
                </button>
                <button onClick={() => setStaffSubTab("DEPARTMENTS")} className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase transition-all flex items-center gap-1.5 ${staffSubTab === "DEPARTMENTS" ? "bg-purple-600 text-white shadow-xs" : "bg-white text-slate-600 border border-slate-200 hover:bg-slate-50"}`}>
                  <Building2 className="w-3.5 h-3.5" /> Department List
                </button>
                <button onClick={() => setStaffSubTab("HISTORY")} className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase transition-all flex items-center gap-1.5 ${staffSubTab === "HISTORY" ? "bg-amber-600 text-white shadow-xs" : "bg-white text-slate-600 border border-slate-200 hover:bg-slate-50"}`}>
              ...
              {(staffSubTab === "REGISTER" || staffSubTab === "ACTIVE" || staffSubTab === "INACTIVE") && (
                <Card className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-2xs">
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-slate-50 border-b border-slate-200">
                        <TableHead className="text-[10px] font-black uppercase text-slate-500">Employee Details</TableHead>
                        <TableHead className="text-[10px] font-black uppercase text-slate-500">ID Code</TableHead>
                        <TableHead className="text-[10px] font-black uppercase text-slate-500">Department</TableHead>
                        <TableHead className="text-[10px] font-black uppercase text-slate-500">Environment</TableHead>
                    ...
                    <TableBody>
                      {staffModuleDataset.length === 0 ? (
                        <TableRow>
                          <TableCell colSpan={6} className="text-center py-6 text-xs text-slate-400 uppercase font-bold">No tracking records found matching parameters</TableCell>
                        </TableRow>
                      ) : (
                        staffModuleDataset.map((emp) => (
                          <TableRow key={emp.staffCode} className="border-b border-slate-100 hover:bg-slate-50/50">
                            <TableCell>
                              <div>
                                <span className="text-xs font-extrabold text-slate-900 block">{emp.fullName}</span>
                                <span className="text-[10px] font-medium text-slate-400 block lowercase">{emp.email}</span>
                              </div>
                            </TableCell>
                            <TableCell className="font-mono text-xs font-bold text-slate-600">{emp.staffCode}</TableCell>
                            <TableCell>
                              <div>
                                <span className="text-xs font-bold text-slate-800 block uppercase">{emp.department}</span>
                                <span className="text-[10px] text-slate-400 block uppercase">{emp.designation}</span>
                              </div>
                            </TableCell>
                            <TableCell>
                              <Badge variant="secondary" className="text-[9px] uppercase font-bold">{emp.workType}</Badge>
                            </TableCell>
                            <TableCell>
                              <Badge className={`text-[9px] font-bold uppercase cursor-pointer ${emp.status === "Active" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-rose-50 text-rose-700 border border-rose-200"}`} onClick={() => toggleStaffStatus(emp.staffCode)}>
                                {emp.status}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-right space-x-2">
                              <Button variant="outline" size="sm" onClick={() => generateEmployeeProfilePDF(emp)} className="h-7 text-[10px] border-blue-200 text-blue-600 font-bold uppercase rounded-md inline-flex items-center gap-1">
              ...
              {staffSubTab === "DEPARTMENTS" && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {departmentMetrics.map((dept) => (
                    <Card key={dept.name} className="bg-white border border-slate-200 rounded-xl p-4 shadow-2xs">
                      <CardHeader className="p-0 pb-2 border-b border-slate-100">
                        <CardTitle className="text-xs font-black text-slate-400 uppercase tracking-widest">{dept.name} Core Division</CardTitle>
                      </CardHeader>
                      <CardContent className="pt-4 p-0 space-y-2">
                        <div className="flex justify-between items-center text-xs">
                          <span className="font-bold text-slate-500 uppercase">Gross Allocations:</span>
                          <span className="font-black text-slate-900">{dept.total} Employees</span>
                        </div>
                        <div className="flex justify-between items-center text-xs">
                          <span className="font-bold text-slate-500 uppercase">Active Status Pool:</span>
                          <span className="font-black text-emerald-600">{dept.active} Active</span>
                        </div>
                      </CardContent>
                    </Card>
                  ))}
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-slate-50/60 border-b border-slate-200">
                        <TableHead className="text-[10px] font-black uppercase text-slate-500">Staff ID Code</TableHead>
                        <TableHead className="text-[10px] font-black uppercase text-slate-500">Calendar Date</TableHead>
                        <TableHead className="text-[10px] font-black uppercase text-slate-500">Day Matrix</TableHead>
                        <TableHead className="text-[10px] font-black uppercase text-slate-500">Logged Entry Time</TableHead>
                        <TableHead className="text-[10px] font-black uppercase text-slate-500">Punch Classification</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {allHistoricalSwipes.slice(0, 50).map((swipe, idx) => (
                        <TableRow key={idx} className="border-b border-slate-100 font-mono text-xs">
                          <TableCell className="font-bold text-blue-600">{swipe.id}</TableCell>
                          <TableCell className="text-slate-600">{swipe.date}</TableCell>
                          <TableCell className="uppercase font-sans text-slate-500 font-bold">{swipe.weekDay}</TableCell>
                          <TableCell className="font-bold text-slate-900">{swipe.time}</TableCell>
                          <TableCell>
                            <Badge variant={swipe.type.toLowerCase().includes("in") ? "default" : "secondary"} className="text-[9px] uppercase font-bold">
                              {swipe.type}
                            </Badge>
                          </TableCell>
                        </TableRow>
                ...
                <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
                  <Card className="bg-white border border-slate-200 rounded-2xl max-w-lg w-full shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
                    <CardHeader className="border-b border-slate-100 p-5 flex flex-row items-center justify-between">
                      <div>
                        <CardTitle className="text-sm font-black text-slate-900 uppercase tracking-wide">Add Employee</CardTitle>
                        <CardDescription className="text-[11px] font-medium text-slate-400 uppercase mt-0.5">Add a new team member to your organization</CardDescription>
                      </div>
                      <button onClick={() => setIsAddModalOpen(false)} className="text-slate-400 hover:text-slate-600 font-bold text-sm">✕</button>
                    </CardHeader>
                    <CardContent className="p-6">
                      <form onSubmit={handleCreateStaffSubmit} className="space-y-5">
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <label className="text-[10px] font-black uppercase text-slate-500 block mb-1">Employee Name *</label>
                            <Input required value={newStaffName} onChange={(e) => setNewStaffName(e.target.value)} placeholder="Sarah Chen" className="text-xs font-bold uppercase" />
                          </div>
                          <div>
                            <label className="text-[10px] font-black uppercase text-slate-500 block mb-1">Email (optional)</label>
                            <Input type="email" value={newStaffEmail} onChange={(e) => setNewStaffEmail(e.target.value)} placeholder="sarah@company.com" className="text-xs font-bold" />
                          </div>
                        </div>

                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <label className="text-[10px] font-black uppercase text-slate-500 block mb-1">Employee ID Code *</label>
                            <Input required value={newStaffCode} onChange={(e) => setNewStaffCode(e.target.value)} placeholder="e.g. GF-004" className="text-xs font-bold uppercase" />
                          </div>
                          <div>
                            <label className="text-[10px] font-black uppercase text-slate-500 block mb-1">Job Designation</label>
                            <Input value={newStaffDesignation} onChange={(e) => setNewStaffDesignation(e.target.value)} placeholder="e.g. UX Designer" className="text-xs font-bold uppercase" />
                          </div>
                        </div>

                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <label className="text-[10px] font-black uppercase text-slate-500 block mb-1">Department</label>
                            <select value={newStaffDept} onChange={(e) => setNewStaffDept(e.target.value)} className="w-full bg-white border border-slate-200 text-xs font-bold uppercase rounded-lg h-9 p-2">
                          ...
                          <div>
                            <label className="text-[10px] font-black uppercase text-slate-500 block mb-1">Cost Center</label>
                            <Input value={newStaffCostCenter} onChange={(e) => setNewStaffCostCenter(e.target.value)} placeholder="HQ" className="text-xs font-bold uppercase" />
                          </div>
                        </div>

                        <div>
                          <label className="text-[10px] font-black uppercase text-slate-500 block mb-1.5">Work Type</label>
                          <div className="grid grid-cols-3 gap-3">
                            {(["In Office", "Remote", "Hybrid"] as const).map((t) => (
                              <div key={t} onClick={() => setNewStaffWorkType(t)} className={`p-3 border rounded-xl cursor-pointer text-center transition-all ${newStaffWorkType === t ? "border-emerald-600 bg-emerald-50/40 text-emerald-900 font-bold" : "border-slate-200 bg-white hover:bg-slate-50 text-slate-600"}`}>
                                <span className="text-[11px] uppercase block tracking-wider font-extrabold">{t}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-end gap-3 pt-2 border-t border-slate-100">
                          <Button type="button" variant="outline" onClick={() => setIsAddModalOpen(false)} className="text-xs font-bold uppercase h-9 rounded-lg">Cancel</Button>
                          <Button type="submit" className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs uppercase h-9 px-4 rounded-lg">Add Employee</Button>
                        </div>
                      </form>
                    </CardContent>
                  </Card>
          ...
          {activeTab === "EMPLOYEES" && (
            <Card className="bg-white border border-slate-200 rounded-xl shadow-xs">
              <CardHeader className="p-4 bg-slate-50/60 border-b border-slate-100 flex flex-row items-center justify-between">
                <div>
                  <CardTitle className="text-xs font-black text-slate-500 uppercase tracking-widest">Staff Members Work Hours Ledger</CardTitle>
                  <div className="text-[10px] text-amber-700 font-bold uppercase mt-1">
                    Rule: If a worker checks in, they are given standard hours for that day regardless of missing check-out.
                  </div>
                </div>
                <div>
                  <select
                  ...
                  >
                    ...
                    {dynamicFilterMenus.map((m) => <option key={m} value={m}>{m}</option>)}
                  </select>
                </div>
              </CardHeader>
              <CardContent className="p-0">
                <Table>
                  <TableHeader>
                    <TableRow className="bg-slate-50 border-b border-slate-200">
                      ...
                      <TableHead className="text-[10px] font-black uppercase text-slate-500">Full Name</TableHead>
                      <TableHead className="text-[10px] font-black uppercase text-slate-500">Department</TableHead>
                      <TableHead className="text-[10px] font-black uppercase text-slate-500 text-right">Regular Work Hours</TableHead>
                      <TableHead className="text-[10px] font-black uppercase text-slate-500 text-right">Overtime Hours</TableHead>
                      <TableHead className="text-[10px] font-black uppercase text-slate-500 text-right">Total Hours Combined</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {filteredViewDataset.map((emp) => (
                      <TableRow key={emp.staffCode} className="border-b border-slate-100 hover:bg-slate-50/50">
                        ...
                        <TableCell className="text-xs font-bold text-slate-900">{emp.fullName}</TableCell>
                        <TableCell className="text-xs text-slate-500 uppercase">{emp.department}</TableCell>
                        <TableCell className="text-xs text-right font-medium">{emp.metrics.totalRegularHours} hours</TableCell>
                        <TableCell className="text-xs text-right font-bold text-blue-600">+{emp.metrics.totalOvertimeHours} hours</TableCell>
                        <TableCell className="text-xs text-right font-black text-slate-900">{emp.metrics.totalHoursSum} hours</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
          ...
          {activeTab === "REPORTS_HUB" && (
            <Card className="bg-white border border-slate-200 rounded-xl shadow-xs">
              <CardHeader className="p-4 border-b border-slate-100">
                <CardTitle className="text-xs font-black text-slate-400 uppercase tracking-widest">Download and Print PDF Documents</CardTitle>
              </CardHeader>
              <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4 p-6">
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 flex flex-col justify-between gap-3">
                  <div>
                    <span className="text-xs font-extrabold uppercase text-slate-800 block">Daily Sheet Report</span>
                    <p className="text-[11px] uppercase text-slate-400 mt-1">Saves a report containing only check-ins made today.</p>
                  </div>
                  <Button onClick={() => generateStaffScopeReport("DAILY")} className="bg-blue-600 text-white font-bold text-xs uppercase h-9">Download Daily PDF</Button>
                </div>
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 flex flex-col justify-between gap-3">
                  <div>
                    <span className="text-xs font-extrabold uppercase text-slate-800 block">Weekly Sheet Summary</span>
                    <p className="text-[11px] uppercase text-slate-400 mt-1">Saves a combined totals summary for this week's work cycles.</p>
                  </div>
                  <Button onClick={() => generateStaffScopeReport("WEEKLY")} className="bg-blue-600 text-white font-bold text-xs uppercase h-9">Download Weekly PDF</Button>
                </div>
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 flex flex-col justify-between gap-3">
                  <div>
                    <span className="text-xs font-extrabold uppercase text-slate-800 block">Full Monthly Record Ledger</span>
                    <p className="text-[11px] uppercase text-slate-400 mt-1">Saves the entire month's clock ledger showing worker total shifts.</p>
                  </div>
                  <Button onClick={() => generateStaffScopeReport("MONTHLY")} className="bg-blue-600 text-white font-bold text-xs uppercase h-9">Download Monthly PDF</Button>
                </div>
              </CardContent>
            </Card>
          )}
                  ...
                  <CardContent className="p-0">
                    {isLoadingUsers ? (
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow className="bg-slate-50 border-b border-slate-200">
                            <TableHead className="text-[10px] font-black uppercase text-slate-500">User Email Identifier</TableHead>
                            <TableHead className="text-[10px] font-black uppercase text-slate-500">Privilege Role Tier</TableHead>
                            <TableHead className="text-[10px] font-black uppercase text-slate-500 text-right">Revoke Account</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          {systemUsers.length === 0 ? (
                            <TableRow>
                              <TableCell colSpan={3} className="text-center py-6 text-xs text-slate-400 uppercase font-bold">
                                No secondary authorization records found.
                              </TableCell>
                            </TableRow>
                          ) : (
                            systemUsers.map((sysUser) => (
                              <TableRow key={sysUser.id} className="border-b border-slate-100 hover:bg-slate-50/50">
                                <TableCell className="font-medium text-xs text-slate-900">{sysUser.email}</TableCell>
                                <TableCell>
                                  <select
                                    value={sysUser.role}
                                    disabled={user?.id === sysUser.id} // Prevent accidental self-demotion
                                    onChange={(e) => handleUpdateUserRole(sysUser.id, e.target.value)}
                                ...
                                <TableCell className="text-right">
                                  <Button
                                    ...
                                    onClick={() => handleDeleteSystemUser(sysUser.id)}
                ...
                <Card className="bg-white border border-slate-200 shadow-sm rounded-xl">
                  ...
                  <CardContent className="space-y-4 text-xs font-medium text-slate-600">
                    <div className="flex gap-3 items-start border-b border-slate-100 pb-3">
                      ...
                      <div>
                        <span className="font-black text-slate-900 block uppercase mb-0.5">ADMINISTRATOR</span>
        ...
        <Card className="bg-white border border-slate-200 rounded-xl shadow-xs">
          <CardHeader className="py-2 px-4 border-b border-slate-100">
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
              <Terminal className="w-3 h-3 text-blue-600" /> System Status Feed Notice
            </span>
          </CardHeader>
