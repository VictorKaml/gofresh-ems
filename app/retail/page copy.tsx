...
/* ------------------------------------------------------------------ */
/* Constants / helpers (ported 1:1 from generate_dashboard.py's HTML) */
/* ------------------------------------------------------------------ */
...
const WD_NAMES: Record<number, string> = { 1: "Sun", 2: "Mon", 3: "Tue", 4: "Wed", 5: "Thu", 6: "Fri", 7: "Sat" };
  ...
  return (
    <div className="panel active">
      ...
      <div className="grid2">
        <div className="card">
          ...
          <div className="chart-box tall">
            <ChartCanvas config={{
              type: "line",
              data: { labels, datasets: [{ data: M.map((m) => m.mwk_per_kg), borderColor: "#0f766e", backgroundColor: "rgba(15,118,110,.12)", tension: 0.3, fill: true, pointRadius: 5, borderWidth: 2 }] },
              options: { maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { callbacks: { label: (c: any) => fmt(Math.round(c.parsed.y)) + " MWK / kg" } } }, scales: { y: { beginAtZero: true, ticks: { callback: (v: any) => fmtM(Number(v)) } } } },
            }} />
          </div>
        </div>
        <div className="card">
          ...
          <div className="chart-box tall">
            <ChartCanvas config={{
              type: "line",
              data: { labels, datasets: [{ data: M.map((m) => m.avg_daily_per_shop), borderColor: "#7c3aed", backgroundColor: "rgba(124,58,237,.12)", tension: 0.3, fill: true, pointRadius: 5, borderWidth: 2 }] },
              options: { maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { callbacks: { label: (c: any) => fmt(Math.round(c.parsed.y)) + " MWK / shop / day" } } }, scales: { y: { beginAtZero: true, ticks: { callback: (v: any) => fmtM(Number(v)) } } } },
            }} />
          </div>
        </div>
      </div>
      <div className="card">
        ...
        <div className="chart-box"><ChartCanvas config={{ type: "bar", data: { labels, datasets: [{ data: M.map((m) => m.n_shops), backgroundColor: "#f59e0b" }] }, options: { maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } } } } }} /></div>
      </div>
    </div>
  );
};
  ...
  <div className="panel active">
    ...
    <div className="grid2">
      <div className="card">
        ...
        <div className="chart-box"><ChartCanvas config={{ type: "bar", data: { labels: D.weekdays.labels, datasets: [{ label: "Revenue", data: D.weekdays.rev, backgroundColor: "#16a34a" }] }, options: { maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { ticks: { callback: (v: any) => fmtM(Number(v)) } } } } }} /></div>
      </div>
      <div className="card">
        <h3>🌍 Revenue by Region</h3>
        <div className="chart-box"><ChartCanvas config={{ type: "doughnut", data: { labels: D.regions.labels, datasets: [{ data: D.regions.data, backgroundColor: PAL }] }, options: { maintainAspectRatio: false, plugins: { legend: { position: "right" } } } }} /></div>
      </div>
    </div>
  </div>
);
  ...
  return (
    <div className="panel active">
      ...
      <div className="card">
        ...
        <table>
          <thead><tr><th>Segment</th><th className="num">Revenue (MWK)</th><th className="num">Qty</th><th className="num">Volume (kg)</th><th className="num">% Share</th><th className="num">Avg Price / Unit</th><th className="num">MWK / kg (NSV)</th></tr></thead>
          <tbody>
            ...
            <tr style={{ background: "#f0fdf4", fontWeight: 700, borderTop: "2px solid #16a34a" }}>
              ...
              <td className="num">{sumQ ? fmt(Math.round(sumR / sumQ)) : "-"}</td>
              <td className="num">{sumW ? fmt(Math.round(sumR / sumW)) : "-"}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
const ShopsPanel: React.FC<{ D: DashboardData }> = ({ D }) => {
  ...
  return (
    <div className="panel active">
      ...
      <div className="card">
        <h3>All Shops by Revenue — Stacked by Segment (largest → smallest)</h3>
        <div style={{ position: "relative", height: 780 }}>
          <ChartCanvas config={{
            ...
            options: {
              ...
              plugins: {
                legend: { position: "top" },
                tooltip: { callbacks: { label: (c: any) => c.dataset.label + ": " + fmt(Math.round(c.parsed.x)) + " MWK", footer: (items: any[]) => { const t = items.reduce((a, i) => a + i.parsed.x, 0); return "Total: " + fmt(Math.round(t)) + " MWK"; } } },
              },
      ...
      <div className="card" style={{ marginTop: 16 }}>
        ...
        <table>
          <thead><tr><th>#</th><th>Shop</th><th className="num">Total</th><th className="num">Chicken</th><th className="num">Beef</th><th className="num">Egg</th><th className="num">Trading</th><th className="num">MWK / kg</th><th className="num">Avg Basket</th><th className="num">Baskets</th></tr></thead>
          <tbody>
            {filtered.map((s, i) => (
              <tr key={s.shop}>
                ...
                <td className="num">{s.mwk_per_kg ? fmt(Math.round(s.mwk_per_kg)) : "-"}</td>
                <td className="num">{s.avg_basket ? fmt(Math.round(s.avg_basket)) : "-"}</td>
...
type ProductSortKey = "rev" | "qty" | "wght" | "mwk_per_kg" | "avg_basket" | "baskets";
const MEDALS = ["🥇", "🥈", "🥉"];
...
const ProductsPanel: React.FC<{ D: DashboardData }> = ({ D }) => {
  ...
  const filtered = useMemo(() => {
    return D.products
      .filter((p) => (!seg || p.seg === seg) && p.name.toLowerCase().includes(q.toLowerCase()))
      .slice()
    ...
    <div className="panel active">
      <div className="filter-row">
        ...
        <select value={seg} onChange={(e) => setSeg(e.target.value)}>
          <option value="">All</option><option>Chicken</option><option>Beef</option><option>Egg</option><option>Trading</option>
        </select>
        <label>Search:</label>
        <input placeholder="Product name…" style={{ width: 240 }} value={q} onChange={(e) => setQ(e.target.value)} />
      </div>
      <div className="card">
        <h3>🏆 All Products by Revenue <span style={{ fontWeight: 400, color: "#64748b", marginLeft: 8 }}>— showing {filtered.length} of {D.products.length} products</span></h3>
        <table>
          <thead><tr><th>#</th><th>Product</th><th>Segment</th><th>Category</th><th className="num">Revenue (MWK)</th><th className="num">Qty</th><th className="num">Volume (kg)</th><th className="num">MWK / kg</th><th className="num">Avg Basket</th><th className="num">Baskets</th><th className="num">% of Total</th></tr></thead>
          <tbody>
            {filtered.map((p, i) => (
              <tr key={p.name}>
                ...
                <td className="num">{p.wght ? fmt(Math.round(p.wght)) : "-"}</td>
                <td className="num">{p.mwk_per_kg ? fmt(Math.round(p.mwk_per_kg)) : "-"}</td>
                <td className="num">{p.avg_basket ? fmt(Math.round(p.avg_basket)) : "-"}</td>
...
const TimePanel: React.FC<{ D: DashboardData }> = ({ D }) => {
  ...
  const hourlyRevDatasets = regionKeys.map((r) => ({ label: r, data: D.region_hour[r].rev, borderColor: regionColor(r, regionKeys), backgroundColor: regionColor(r, regionKeys) + "22", tension: 0.3, fill: false, pointRadius: 3, borderWidth: 2 }));
  const hourlyQtyDatasets = regionKeys.map((r) => ({ label: r, data: D.region_hour[r].qty, borderColor: regionColor(r, regionKeys), backgroundColor: regionColor(r, regionKeys) + "22", tension: 0.3, fill: false, pointRadius: 3, borderWidth: 2 }));

  return (
    <div className="panel active">
      <div className="insight">💡 Peak selling hour is typically around midday. Most revenue is generated between 09:00 and 17:00. Region overlays reveal where each area&rsquo;s rhythm differs.</div>
      <div className="grid2">
        <div className="card">
          <h3>Hourly Revenue Pattern — by Region</h3>
          <div className="chart-box tall"><ChartCanvas config={{ type: "line", data: { labels: D.hours.labels, datasets: hourlyRevDatasets }, options: { maintainAspectRatio: false, plugins: { legend: { position: "top" }, tooltip: { callbacks: { label: (c: any) => c.dataset.label + ": " + fmt(Math.round(c.parsed.y)) + " MWK" } } }, scales: { y: { ticks: { callback: (v: any) => fmtM(Number(v)) } } } } }} /></div>
        </div>
        <div className="card">
          <h3>Hourly Volume Pattern — by Region</h3>
          <div className="chart-box tall"><ChartCanvas config={{ type: "line", data: { labels: D.hours.labels, datasets: hourlyQtyDatasets }, options: { maintainAspectRatio: false, plugins: { legend: { position: "top" }, tooltip: { callbacks: { label: (c: any) => c.dataset.label + ": " + fmt(Math.round(c.parsed.y)) + " units" } } } } }} /></div>
        </div>
      </div>
      <div className="card">
        ...
        <div className="chart-box tall">
          <ChartCanvas config={{
            data: { labels: D.days.labels, datasets: [{ type: "bar", label: "Revenue (MWK)", data: D.days.rev, backgroundColor: "#16a34a", yAxisID: "y" }, { type: "line", label: "Quantity", data: D.days.qty, borderColor: "#dc2626", backgroundColor: "#dc2626", yAxisID: "y1", tension: 0.3 }] },
            options: { maintainAspectRatio: false, scales: { y: { position: "left", ticks: { callback: (v: any) => fmtM(Number(v)) } }, y1: { position: "right", grid: { display: false } } } },
          }} />
        </div>
      </div>
      <div className="card" style={{ marginTop: 16 }}>
        <h3>Hourly Pattern per Shop — Revenue (bars) + Avg Basket Size (line)</h3>
        <div style={{ fontSize: 11, color: "#64748b", marginBottom: 10 }}>All shops, sorted by total revenue. Green bars = MWK revenue per hour. Orange line = avg basket size (MWK per unique invoice) that hour.</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: 12 }}>
          {D.shop_hour.map((s) => (
            <div key={s.shop} style={{ background: "#fafafa", border: "1px solid #e2e8f0", borderRadius: 8, padding: "8px 10px" }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "#0f172a", marginBottom: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }} title={s.shop}>{s.shop}</div>
              <div style={{ fontSize: 10, color: "#64748b", marginBottom: 6 }}>Total: {fmt(Math.round(s.total))} MWK</div>
              <div style={{ position: "relative", height: 150 }}>
                <ChartCanvas config={{
                  data: { labels: D.hours.labels, datasets: [{ type: "bar", label: "Revenue", data: s.rev, backgroundColor: "#16a34a", yAxisID: "y", order: 2 }, { type: "line", label: "Avg Basket", data: s.avg_basket, borderColor: "#f59e0b", backgroundColor: "#f59e0b", yAxisID: "y1", tension: 0.3, pointRadius: 2, borderWidth: 2, order: 1, spanGaps: true }] },
                  options: { maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { callbacks: { label: (c: any) => c.dataset.label + ": " + fmt(Math.round(c.parsed.y)) + " MWK" } } }, scales: { x: { ticks: { font: { size: 8 }, maxRotation: 0, autoSkip: true, maxTicksLimit: 8 } }, y: { position: "left", ticks: { font: { size: 9 }, callback: (v: any) => fmtM(Number(v)) } }, y1: { position: "right", grid: { display: false }, ticks: { font: { size: 9 }, callback: (v: any) => fmtM(Number(v)) } } } },
                }} />
              </div>
            </div>
          ))}
...
const RegionsPanel: React.FC<{ D: DashboardData }> = ({ D }) => {
  ...
  return (
    <div className="panel active">
      <div className="grid2">
        <div className="card"><h3>Revenue by Region</h3><div className="chart-box tall"><ChartCanvas config={{ type: "pie", data: { labels: D.regions.labels, datasets: [{ data: D.regions.data, backgroundColor: PAL }] }, options: { maintainAspectRatio: false, plugins: { legend: { position: "right" } } } }} /></div></div>
        <div className="card"><h3>Quantity by Region</h3><div className="chart-box tall"><ChartCanvas config={{ type: "pie", data: { labels: D.regions.labels, datasets: [{ data: D.regions.qty, backgroundColor: PAL }] }, options: { maintainAspectRatio: false, plugins: { legend: { position: "right" } } } }} /></div></div>
      </div>
      <div className="card">
        <h3>Region Performance</h3>
        <table>
          <thead><tr><th>Region</th><th className="num"># Shops</th><th className="num">Revenue (MWK)</th><th className="num">Qty</th><th className="num">Avg Sales / Shop</th><th className="num">% Share</th></tr></thead>
          <tbody>
            {D.regions.labels.map((r, i) => {
              ...
              return (
                <tr key={r}>
                  ...
                  <td className="num">{t ? ((rev / t) * 100).toFixed(1) : "0.0"}%</td>
                </tr>
              );
            })}
            <tr style={{ background: "#f0fdf4", fontWeight: 700, borderTop: "2px solid #16a34a" }}>
              ...
              <td className="num">{fmt(Math.round(sumR))}</td>
              <td className="num">{fmt(sumQ)}</td>
              <td className="num">{sumS ? fmt(Math.round(sumR / sumS)) : "-"}</td>
              <td className="num">100.0%</td>
            </tr>
          </tbody>
        </table>
...
const DataPanel: React.FC<{ D: DashboardData }> = ({ D }) => (
  <div className="panel active">
    <div className="card">
      <h3>Daily Summary</h3>
      <table>
        <thead><tr><th>Date</th><th className="num">Revenue (MWK)</th><th className="num">Qty</th></tr></thead>
        <tbody>{D.days.labels.map((d, i) => (<tr key={d}><td>{d}</td><td className="num">{fmt(Math.round(D.days.rev[i]))}</td><td className="num">{fmt(D.days.qty[i])}</td></tr>))}</tbody>
      </table>
    </div>
    <div className="card" style={{ marginTop: 16 }}>
      <h3>Hourly Summary</h3>
      <table>
        <thead><tr><th>Hour</th><th className="num">Revenue (MWK)</th><th className="num">Qty</th></tr></thead>
        <tbody>{D.hours.labels.map((h, i) => (<tr key={h}><td>{h}</td><td className="num">{fmt(Math.round(D.hours.rev[i]))}</td><td className="num">{fmt(D.hours.qty[i])}</td></tr>))}</tbody>
      </table>
    </div>
  </div>
);
...
function defaultRange(dates: string[]): { start: string; end: string } {
  if (!dates.length) return { start: "", end: "" };
  const dataStart = dates[0];
  const dataEnd = dates[dates.length - 1];
  const yearStart = dataStart.slice(0, 4) + "-01-01";
  return { start: dataStart < yearStart ? dataStart : yearStart, end: dataEnd };
}
...
const GoFreshSalesDashboard: React.FC = () => {
  ...
  async function loadData() {
    ...
    setLoadedRows(0);
    try {
      const ds = await fetchRetailDataset(supabase, (n) => setLoadedRows(n));
      setDataset(ds);
      setDatasetVersion((v) => v + 1);
      const { start, end } = defaultRange(ds.lookups.dates);
      setDateStart(start);
      setDateEnd(end);
    } catch (err: any) {
  ...
  const D = useMemo(() => {
    if (!dataset || !dateStart || !dateEnd) return null;
    return computeD(dataset.lookups, dataset.rows, dateStart, dateEnd);
  }, [dataset, dateStart, dateEnd]);

  const renderKey = `${datasetVersion}-${dateStart}-${dateEnd}`;
  ...
  function handleReset() {
    if (!dataset) return;
    const { start, end } = defaultRange(dataset.lookups.dates);
    setDateStart(start);
    setDateEnd(end);
  }

  function handleQuickRange(days: number) {
    if (!dataset || dataset.lookups.dates.length === 0) return;
    const dates = dataset.lookups.dates;
    ...
    <div className="gf-dashboard">
      <style>{`
        ...
        .gf-dashboard {
          ...
          background: #f4f6f8; color: #1f2937; line-height: 1.4; min-height: 100vh;
        }
      ...
      <header>
        ...
        <div className="sub">
          {D ? `${D.date_range} · ${fmt(D.totals.rows)} transactions · ${D.totals.n_shops} shops · ${D.totals.n_days} days` : loading ? "Loading data from Supabase…" : "No data loaded"}
        </div>
      ...
      <div className="container">
        ...
        {loadError && <div className="banner error">Couldn&rsquo;t load data: {loadError} — <a href="#" onClick={(e) => { e.preventDefault(); loadData(); }}>retry</a></div>}
        {loading && <div className="banner info">Loading rows from Supabase… {loadedRows ? `(${fmt(loadedRows)} so far)` : ""}</div>}

        {!loading && dataset && dataset.rows.length === 0 && !loadError && (
          <div className="banner info">No rows in retail_sales yet — upload an Excel file to get started.</div>
        )}
        ...
        {D && (
          <>
            <div className="filter-bar">
              ...
              <button className="quick-btn" onClick={() => handleQuickRange(1)}>Last 1 day</button>
              <button className="quick-btn" onClick={() => handleQuickRange(3)}>Last 3 days</button>
              <button className="quick-btn" onClick={() => handleQuickRange(7)}>Last 7 days</button>
            ...
            {tab === "exec" && <ExecutivePanel key={renderKey} D={D} />}
            {tab === "overview" && <OverviewPanel key={renderKey} D={D} />}
            {tab === "segments" && <SegmentsPanel key={renderKey} D={D} />}
            {tab === "shops" && <ShopsPanel key={renderKey} D={D} />}
            {tab === "products" && <ProductsPanel key={renderKey} D={D} />}
            {tab === "time" && <TimePanel key={renderKey} D={D} />}
            {tab === "regions" && <RegionsPanel key={renderKey} D={D} />}
            {tab === "data" && <DataPanel key={renderKey} D={D} />}
          </>
        )}

        <div className="footer">
          GoFresh RetailMax Sales Intelligence · Live from Supabase (retail_sales) · Open in any modern browser (Chrome, Edge, Safari, Firefox)
        </div>
      </div>
    </div>
  );
